...
import { PlusCircle, Trash2, Save, X, FileText, Receipt, Package, Truck, User, Activity } from "lucide-react"
...
export default function EditVendorBillPage() {
    ...
    React.useEffect(() => {
        async function fetchData() {
            try {
                const [jobRes, vendorRes, billRes] = await Promise.all([
                    fetch("/api/jobs"),
                    fetch("/api/companies?type=Vendor"),
                    fetch(`/api/vendor-bills/${id}`)
                ])
    ...
    async function onSubmit(data: VendorBillFormValues) {
        ...
        try {
            ...
            toast.success("Vendor Goods Bill Updated!")
        ...
        <div className="bg-slate-50 min-h-screen pb-20">
            ...
            <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 h-16 flex items-center justify-between px-10 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold uppercase tracking-widest text-slate-600">Edit Vendor Goods Bill: {id}</span>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <Button onClick={handleSubmit(onSubmit)} disabled={isSubmitting} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-8 shadow-lg shadow-emerald-200">
                        <Save size={18} className="mr-2" /> {isSubmitting ? "Updating..." : "Update Bill"}
                    </Button>
                </div>
            </div>
