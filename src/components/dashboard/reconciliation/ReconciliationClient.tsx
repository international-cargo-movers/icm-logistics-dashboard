...
export default function ReconciliationClient({ initialData }: ReconciliationClientProps) {
    ...
    return (
        <Card className="border-none shadow-2xl shadow-slate-200/50 bg-white overflow-hidden rounded-3xl">
            <div className="px-8 py-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between bg-slate-50/50 gap-4">
                <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                    <Scale className="h-5 w-5 text-blue-600" /> Job-by-Job Reconciliation
                </h3>
                ...
                <Table>
                    <TableHeader className="bg-white">
                        <TableRow className="hover:bg-transparent border-slate-100">
                            ...
                            <TableHead className="text-right font-bold text-slate-400 uppercase text-[10px] tracking-widest">Receivables</TableHead>
                            <TableHead className="text-right font-bold text-slate-400 uppercase text-[10px] tracking-widest">Payables</TableHead>
                        ...
                        {filteredData.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={6} className="text-center py-12 text-slate-400 font-medium">
                                    No reconciliation records found matching "{searchTerm}"
                                </TableCell>
                            </TableRow>
                        ) : (
                            filteredData.map((data) => (
                                <TableRow key={data.id} className="group hover:bg-slate-50/80 transition-colors border-slate-50">
                                    ...
                                    <TableCell className="text-right">
                                        <span className="font-bold text-emerald-600">{formatCurrency(data.incoming)}</span>
                                    </TableCell>
                                    <TableCell className="text-right">
                                        <span className="font-bold text-rose-600">{formatCurrency(data.outgoing)}</span>
                                    </TableCell>
