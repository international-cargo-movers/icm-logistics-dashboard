...
export default function PartiesSection() {
  ...
  // --- PEEK MODAL STATE ---
  const [activeModal, setActiveModal] = React.useState<"shipper" | "consignee" | null>(null)
  ...
  // --- MODAL HANDLERS ---
  const openPeekModal = (type: "shipper" | "consignee") => {
    const fieldName = type === "shipper" ? "shipperId" : "consigneeId"
    const currentValue = getValues(`partyDetails.${fieldName}`)
    const companyName = type === "shipper" ? searchShipper : searchConsignee
    
    // Check if it's an existing company in our DB
    ...
    if (existingCompany) {
      ...
      // It's a brand new "stub" being typed out
  ...
  const handleModalSave = async () => {
    // In a production app, you would POST/PUT this to your /api/companies route right here,
    // get the new MongoDB _id back, and swap it into the form state.
    // For now, we simulate saving the rich data and closing the modal.
    console.log(`Saving rich data for ${activeModal}:`, modalData)
    
    // Auto-update the combobox text to match any name changes made in the modal
    if (activeModal === "shipper") {
        setSearchShipper(modalData.name)
        setValue("partyDetails.shipperId", modalData.name) // Triggers your onSubmit interceptor later
    } else {
...
<ComboboxInput 
                      ...
                      onBlur={() => {
                        // Save the text to the form when clicking away / tabbing
                        if (searchShipper) field.onChange(searchShipper)
                      }}
                      onKeyDown={(e) => {
                        // Save the text to the form when hitting Enter
                        if (e.key === 'Enter') {
                          e.preventDefault() 
                          e.stopPropagation() 
                          if (searchShipper) {
                            field.onChange(searchShipper)
...
<ComboboxInput 
                      ...
                      onBlur={() => {
                        // Save the text to the form when clicking away / tabbing
                        if (searchConsignee) field.onChange(searchConsignee)
                      }}
                      onKeyDown={(e) => {
                        // Save the text to the form when hitting Enter
                        if (e.key === 'Enter') {
                          ...
                          e.stopPropagation() 
                          if (searchConsignee) {
                            field.onChange(searchConsignee)
          ...
          <div className="bg-surface-container-lowest w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-outline-variant/20 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold">
                    {activeModal === "shipper" ? "Shipper Details" : "Consignee Details"}
                  </p>
                </div>
              </div>
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <input 
                  type="text" value={modalData.taxId} onChange={e => setModalData({...modalData, taxId: e.target.value})}
                  className="w-full bg-surface-container-low border-none rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                />
              </div>
              <div>
                ...
                <input 
                  type="text" value={modalData.streetAddress} onChange={e => setModalData({...modalData, streetAddress: e.target.value})}
                  className="w-full bg-surface-container-low border-none rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    type="text" value={modalData.city} onChange={e => setModalData({...modalData, city: e.target.value})}
                    className="w-full bg-surface-container-low border-none rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" value={modalData.state} onChange={e => setModalData({...modalData, state: e.target.value})}
                    className="w-full bg-surface-container-low border-none rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" value={modalData.zipCode} onChange={e => setModalData({...modalData, zipCode: e.target.value})}
                    className="w-full bg-surface-container-low border-none rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" value={modalData.country} onChange={e => setModalData({...modalData, country: e.target.value})}
                    className="w-full bg-surface-container-low border-none rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                  />
                </div>
              </div>
            ...
            <div className="p-4 bg-surface-container flex justify-end gap-3 border-t border-surface-container-highest">
              <button type="button" onClick={() => setActiveModal(null)} className="px-5 py-2.5 text-sm font-bold text-on-surface-variant hover:bg-surface-container-low rounded-lg transition-colors">
                Cancel
              </button>
              <button type="button" onClick={handleModalSave} className="px-5 py-2.5 text-sm font-bold bg-primary text-on-primary rounded-lg hover:opacity-90 transition-opacity">
                Apply to Shipment
              </button>
            </div>
          </div>
